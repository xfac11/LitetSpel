...
bool GBuffer::setRenderTargets(ID3D11Device *& device, ID3D11DeviceContext * deviceContext)
{
	...
	deviceContext->PSSetShaderResources(0, 4, null);
...
void GBuffer::clear(ID3D11DeviceContext * deviceContext, float color[4])
{
	...
	for (int i = 0; i < GBUFFERCAP; i++)
	{
		deviceContext->ClearRenderTargetView(this->renderTars[i], black);
	}
	deviceContext->ClearRenderTargetView(this->renderTars[1], color);
	deviceContext->ClearDepthStencilView(depthStencView, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 1);
}
