...
bool TextureLoad::Initialize(const char* filename, int mipLevels)
{
	bool result;
	int height, width;
	...
	hResult = System::getDevice()->CreateTexture2D(&textureDesc, NULL, &m_texture);
	if (FAILED(hResult))
	{
		return false;
	}
	...
	System::getDeviceContext()->UpdateSubresource(m_texture, 0, NULL, m_targaData, rowPitch, 0);
	...
	System::getDeviceContext()->GenerateMips(m_textureView);
